...
possui células ou não. Hoje em dia, 
sabe-se que as características deles ficam salvas dentro de uma molécula, 
...
é a transcrição do RNA (Ácido Ribonucleico) a partir de um DNA. 
Este processo consiste em identificar as bases nitrogenadas que 
formam o DNA, em questão. Todas as moléculas de DNA podem ser 
representadas como uma sequência de bases nitrogenadas que podem ser:
 a A (adenina), T (timina), G (guanina) ou C (citosina). Abaixo, 
 há um exemplo disto
...
Para os exemplos acima, a entrada seria `"ATTGCTGCGCATTAACGACGCGTA"`
 ...
 function dnaParaRna(dna:string) :string {
     ...
     const trocaDasBases = dna[i]
     
     switch(trocaDasBases){
        case "A":
            rna += "U"
                 break;
        case "T":
